...
export async function getCruxData(origin: string, formFactor: FormFactor): Promise<CruxData> {
  const body = {
    origin,
    formFactor
  }

  const res = await fetch(`https://chromeuxreport.googleapis.com/v1/records:queryHistoryRecord?key=${process.env.GOOGLE_API_KEY}`, {
    method: 'POST',
...
export function transformToChartData(data: CruxData, metric: string): CruxChart {
  const labels = data.record.collectionPeriods.map(p => `${p.lastDate.year}-${p.lastDate.month}-${p.lastDate.day}`)
